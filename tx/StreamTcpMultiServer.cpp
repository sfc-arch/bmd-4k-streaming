...
int main(int argc, char *argv[]) {
  ...
  int video_yes = 1;
  int video_srv_sock;
  struct sockaddr_in video_srv_addr;
  struct sockaddr_in video_cli_addr;
  socklen_t video_cli_addr_len = sizeof(video_cli_addr);

  int audio_yes = 1;
  int audio_srv_sock;
  struct sockaddr_in audio_srv_addr;
  struct sockaddr_in audio_cli_addr;
  socklen_t audio_cli_addr_len = sizeof(audio_cli_addr);

  // Video Server
  video_srv_addr.sin_family = AF_INET;
  video_srv_addr.sin_port = htons(62310);
  // video_srv_addr.sin_port = htons(atoi(argv[1]));
  video_srv_addr.sin_addr.s_addr = INADDR_ANY;
  video_srv_sock = socket(AF_INET, SOCK_STREAM, 0);
  setsockopt(video_srv_sock, SOL_SOCKET, SO_REUSEADDR, (const char *)&video_yes, sizeof(video_yes));
  bind(video_srv_sock, (struct sockaddr *)&video_srv_addr, sizeof(video_srv_addr));
  listen(video_srv_sock, 5);

  // Audio Server
  audio_srv_addr.sin_family = AF_INET;
  audio_srv_addr.sin_port = htons(62311);
  // audio_srv_addr.sin_port = htons(atoi(argv[2]));
  audio_srv_addr.sin_addr.s_addr = INADDR_ANY;
  audio_srv_sock = socket(AF_INET, SOCK_STREAM, 0);
  setsockopt(audio_srv_sock, SOL_SOCKET, SO_REUSEADDR, (const char *)&audio_yes, sizeof(audio_yes));
  bind(audio_srv_sock, (struct sockaddr *)&audio_srv_addr, sizeof(audio_srv_addr));
  listen(audio_srv_sock, 5);

  // Client Wait
  g_video_sock = accept(video_srv_sock, (struct sockaddr *)&video_cli_addr, &video_cli_addr_len);
  g_audio_sock = accept(audio_srv_sock, (struct sockaddr *)&audio_cli_addr, &audio_cli_addr_len);
