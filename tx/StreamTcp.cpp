...
enum STREAM_TYPE { T_STREAM_VIDEO, T_STREAM_AUDIO };
struct stream_info {
...
HRESULT DeckLinkCaptureDelegate::VideoInputFrameArrived(IDeckLinkVideoInputFrame* videoFrame, IDeckLinkAudioInputPacket* audioFrame)
{
  ...
  if (videoFrame)
  {
    ...
    else
    {
      ...
      // 7680*8
      printf("Frame received (#%lu) %lu %lux%lu [%s] - %s - Size: %li bytes\n",
        g_frameCount,
      ...
      send(g_sock, frameBytes, videoFrame->GetRowBytes() * videoFrame->GetHeight(), 0);
  ...
  if (audioFrame)
  {
    ...
    send(g_sock, &info, sizeof(stream_info), 0);
    send(g_sock, audioFrameBytes, audioFrame->GetSampleFrameCount() * g_config.m_audioChannels * (g_config.m_audioSampleDepth / 8), 0);
...
int main(int argc, char *argv[]) {
  ...
  g_addr.sin_port = htons(62309);
  g_addr.sin_addr.s_addr = inet_addr("192.168.100.41");
